...
function DHTNode(config) {
  ...
  this.transport.before('serialize', function (message,next) {
    console.log("message: "+JSON.stringify(message)); next();
  });
  this.transport.before('send', encrypt.hooks.verify);

  // handle errors from RPC
  this.transport.on('error', function(err) {
    that.logger.warn('RPC error raised, reason: %s', err.message);
  });
}
